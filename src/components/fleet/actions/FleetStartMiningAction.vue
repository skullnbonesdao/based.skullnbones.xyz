...
async function sendTx() {
  ...
  try {
    staratlasIxs.push(...(await gameInstructionHandler.startMining(props.fleet)))
    if (staratlasIxs.length > 0)
      await handleStarAtlasTransaction(
        `Instructions start mining`,
        staratlasIxs,
        signer,
...
<template>
  <q-btn color="primary" icon="play_for_work" @click.prevent="sendTx">
    <q-tooltip>Start mining</q-tooltip>
  </q-btn>
</template>
